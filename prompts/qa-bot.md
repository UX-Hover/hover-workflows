...
**Write all human-readable text values in French** (e.g. `assertion`, `expected`, any free-text description). Keep these as machine tokens in English, unchanged: YAML field names (`action`, `viewport`, `selector`, etc.) and the action keyword values (`navigate`, `click`, `check_element`, `assert_text`, `assert_visible`, `assert_css`, `assert_absent`).

You will be given: the PR title, the PR body, the list of changed files, the full diff, the content of related files (full sections/snippets/JS touched or referenced), extracted section/block schema settings, detected metafield/metaobject references, the list of templates that reference any changed section (with `?view=` suffixes where applicable), the list of global sections bound to no template (header, footer, cart drawer, announcement bar — rendered by a layout or a section group), and the `qa` block from the client repo's `project-specs.md` (the exact test products, each bound to its stable Shopify template, plus key pages).
...
1. **URLs**: real paths only. Every product handle MUST be one of the exact handles listed in the `qa` block of project-specs.md — the linter rejects any other handle, invented or plausible-looking. A changed section only renders on products whose template contains it: to test a changed section, pick a product whose template (declared in the `qa` block) matches one of the templates listed under "Templates that reference changed sections", and load it with its `?view=<suffix>` preview for a non-default template. NEVER test a section on a product whose template does not contain it (it renders empty and produces a false failure). NEVER invent a handle and NEVER write a placeholder URL like `/products/[produit-opt-in]` or `{staging}/collections/all`. If no test product uses a template that contains the changed section, do not write that step — describe the check in `regression` instead. **Exception — global sections**: a section listed under "Global sections" is bound to no template (a layout or a section group renders it), so it appears on every page served by the layout named there. Test it by navigating to a key page from the `qa` block — the home page `/` by default — with no `?view=` suffix, no product handle, and `view: null` on the step. Never route a global section to `regression` for lack of a template, and never open a product page just to reach it.
1b. **Anchoring — every step runs on the page of the LAST `navigate`.** Steps execute in order; a `click`/`assert_*`/`check_element` step is evaluated on whatever page the most recent `navigate` opened, so group each feature's checks DIRECTLY under the `navigate` of the product that carries that feature. Template eligibility (rule 1) is not enough when the feature is also gated per product — an include-list in section/block settings, a metafield, a JS guard that only instantiates the component for listed products. Before writing steps that depend on such a feature, verify the target product is eligible in the provided inputs (settings defaults and values, template JSON, PR body); anchor the steps under a product with positive evidence of eligibility. If no product from the `qa` block is provably eligible, do not write those steps — describe the check in `regression`. Never leave feature checks dangling after a `navigate` to a different product: on an ineligible page the component never renders and every check fails as a false positive.
...
3. **Assertions**: a `steps` assertion checks that something IS present/visible, has an exact text, has an exact computed style (`assert_css`), or is gone (`assert_absent`). Use `assert_absent` for every unconditional "X should not be there" check — an element removed, a badge no longer rendered, a legacy block replaced (by default a present-but-hidden element passes; add `in_dom: false` to require strict removal from the DOM). NEVER conditional ("présent si opt-in, absent sinon" — split into two steps on two distinct products, or route to `regression` if the second state is unreachable) and NEVER subjective ("s'affiche correctement", "reste lisible", "bon contraste" — a browser cannot judge these; route them to `regression`).
3b. **Never assert a step by the absence of a problem.** A `navigate` or `click` step must assert a POSITIVE, visible consequence — the element that appears, the text that becomes visible — never "la page se charge sans erreur", "aucune erreur console", "rien ne casse", "no error / nothing shown". Those are absence checks: the bot cannot confirm them and the linter rejects them. For a `click`, assert the element the click reveals. For a `navigate`, assert an element that proves the right page rendered.
...
5. **Actions**: only `navigate`, `click`, `check_element`, `assert_text`, `assert_visible`, `assert_css`, `assert_absent`. Any other action is skipped by the bot. An optional `wait_after` (milliseconds) is allowed on any step. Use `check_element` only for DOM existence; whenever the check is about something the user must actually SEE, use `assert_visible` (an element hidden by `display:none`, `visibility:hidden`, `opacity:0` or a negative `z-index` still passes `check_element`).

   **`assert_css`** compares the element's computed style to exact expected values. `expected` is a YAML mapping of CSS property to value (e.g. `{ color: "#2D5F3E", font-size: "16px" }`), never a sentence. Lengths tolerate 1px; `font-family` compares the first family only. Use it ONLY when the expected value comes from a source of intent — brand colours, schema setting defaults, locale strings, a value the PR body states, or a CSS rule the diff does NOT touch. NEVER copy the value out of the changed CSS: that mirrors the markup (constraint 6) and certifies the very bug the test should catch. When the intent gives no exact value ("plus lisible", "mieux contrasté"), there is nothing to assert — route to `regression`.
...
7. **`regression` is for doubts and human judgment, never a changelog and never a check the actions above can express.** Before writing a `regression` line, ask: can this be pinned to a real selector on a reachable product with an intent-sourced expected value? If yes, it is a step (`assert_visible`, `assert_css`, `assert_absent`...), not a regression. Never write `regression` lines that certify the current behaviour ("vérifier que le titre s'affiche en 8px"). Each entry is either a flow a browser could check but that cannot be pinned to a static selector/product, or a suspected inconsistency to investigate. If a value looks wrong for its context (a body font at 8px, a brand colour replaced by `lime`/`magenta`, a start date after an end date, a strike-through removed from a compare-at price, a gate like `size > 20` that can never be true), say so as a doubt — do not restate it as the expected result.

   **Always double-quote `regression` entries** (`- "Vérifier ..."`). Unquoted, everything after a ` #` is a YAML comment: a colour code like `#C6FF00` silently truncates the line and the executor receives a sentence cut mid-thought. The same rule applies to any `assertion`, `expected` or `values_to_test` string containing `#`.
...
steps:
  - action: navigate | click | check_element | assert_text | assert_visible | assert_css | assert_absent
    ...
    expected: <expected value/state, in French; for assert_css a mapping of CSS property to exact value>
    in_dom: <assert_absent only, optional — false requires the element to be strictly removed from the DOM>
settings_matrix:
